...
ATF_TC_BODY(semtimedop_basic, tc)
{
    ...
    ATF_REQUIRE_MSG(val == 1, "Semaphore value incorrect: got %d, expected 1", val);

    /* Clean up */
    ATF_REQUIRE_MSG(semctl(semid, 0, IPC_RMID) != -1, "semctl IPC_RMID failed: %s", strerror(errno));
}
...
{
    atf_tc_set_md_var(tc, "descr", "semtimedop blocks until timeout expires");
}
...
ATF_TC_BODY(semtimedop_timeout, tc)
{
    ...
    if (pid == 0) {
        /* Child: perform semtimedop with negative sem_op, should block until timeout */
        sops.sem_num = 0;
        sops.sem_op = -1;
        sops.sem_flg = 0;
...
{
    atf_tc_set_md_var(tc, "descr", "semtimedop with SEM_UNDO adjusts semaphore on exit");
}
...
ATF_TC_BODY(semtimedop_semundo, tc)
{
    ...
    if (pid == 0) {
        ...
        _exit(0); /* Exit normally, SEM_UNDO should be triggered */
    } else {
        ...
        ATF_REQUIRE_MSG(val == 0, "Semaphore value incorrect after SEM_UNDO: got %d, expected 0", val);
...
{
    atf_tc_set_md_var(tc, "descr", "semtimedop handles invalid parameters correctly");
}
...
ATF_TC_BODY(semtimedop_invalid, tc)
{
    ...
    ATF_REQUIRE_MSG(semtimedop(-1, &sops, 1, &timeout) == -1 && errno == EINVAL,
        "semtimedop did not fail on invalid semid");
    ...
    ATF_REQUIRE_MSG(semtimedop(semid, &sops, 1, &timeout) == -1 && errno == EFBIG,
